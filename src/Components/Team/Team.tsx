    ...
    <div>
          ...
          <Input
            ...
            onChange={(e) => {
              ...
              if (!myRagamId.match(re))
                setAddStatus({ type: 1, value: "Enter a valid Ragam ID" });
                    ...
                    (result) => {
                      //verify the result
                       if (result.message == "Invalid ID") {
                        setAddStatus({
                          type: 1,
                          value: "No user found with Ragam ID " + myRagamId,
                        });
                        ...
                        setAddStatus({
                          type: 3,
                          value: "User found: " + result.name +" ( " + result.collegeName +" )",
                        });
                        setNUserObj({
                          id: result.id,
                          name: result.name,
            ...
            <Button
              ...
              onClick={() => {
                if (dayjs(props.regEndDate).diff(dayjs()) < 0) {
                  setAddStatus({
                    type: 1,
                    value: "Cannot edit team. Registration has ended",
                  });
                  return;
                }
                  ...
                  .then(
                    (result) => {
                      ...
                      if (result.statusCode == 400) {
                        ...
                        setNUser(-2);
                        message.success("Added teammate successfully");
                      }
                      console.log(result);
                    },
                    (error) => {
        ...
        dayjs(props.regEndDate).diff(dayjs()) > 0 && (
          <Button
            ...
            onClick={() => {
              if (dayjs(props.regEndDate).diff(dayjs()) < 0) {
                message.error(
                  "Cannot edit team. Registration has ended",
                  5
                );
                return;
              }
      ...
      {dayjs(props.regEndDate).diff(dayjs()) > 0 && (
        <Alert
          style={{ marginTop: "15px", marginBottom: "15px" }}
          message={
            "You won't be able to edit the team after registration ends ( " +
            dayjs(props.regEndDate).format("DD MMMM hh mm a") +
            " )"
          }
          showIcon
          type="info"
